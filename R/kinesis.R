...
#' @param data the data blob (<1 MB) to put into the record, which is
#'     base64-encoded when the blob is serialized
#' @param partition_key Unicode string with a maximum length limit of
#'     256 characters determining which shard in the stream the data
#'     record is assigned to
#' @param ... optional further parameters, such as
#'     \code{ExplicitHashKey} or \code{SequenceNumberForOrdering}
#' @export
#' @return list of \code{ShardId}, \code{SequenceNumber} and
#'     \code{EncryptionType}
#' @references
...
#' @param shard the shard ID of the Kinesis Data Streams shard to get
#'     the iterator for
#' @param shard_iterator_type determines how the shard iterator is
#'     used to start reading data records from the shard
#' @param ... optional further parameters, such as
#'     \code{StartingSequenceNumber} or \code{Timestamp}
#' @export
...
kinesis_get_shard_iterator <- function(stream, shard, shard_iterator_type = c(
    'TRIM_HORIZON', 'LATEST',
...
#' @param shard_iterator the position in the shard from which you want
#'     to start sequentially reading data records, usually provided by
#'     \code{\link{kinesis_get_shard_iterator}}
#' @param limit maximum number of records to return
#' @export
#' @return list of \code{Records}, \code{NextShardIterator} and
#'     \code{MillisBehindLatest}
#' @references
